...
class BoardUsedException(BoardException):
    def __str__(self):
        return "Вы стреляли по этим координатам"
...
class Ship:
    def __init__(self, bow, lb, position):
        self.bow = bow
        self.lb = lb
    ...
    def random_place(self):
        ...
        for lb in lens:
            while True:
                ...
                if attempts > 2000:
                    return None
                ship = Ship(Dot(random.randint(0, self.size), random.randint(0, self.size)), lb, random.randint(0, 1))
                try:
                    board.add_ship(ship)
                    break
